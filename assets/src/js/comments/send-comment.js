...
function postComment(){
	...
	argonAjax("ajax_post_comment", {
		...
		if (result.status == "failed"){
			iziToast.destroy();
			toastError(__("评论发送失败"), result.msg);
			return;
		}

		//发送成功
		iziToast.destroy();
		toastSuccess(__("发送成功"), __("您的评论已发送"));
		//插入新评论
...
function editComment(){
	...
	argonAjax("user_edit_comment", {
		...
		if (result.status == "failed"){
			iziToast.destroy();
			toastError(__("评论编辑失败"), result.msg);
			return;
		}
		...
		iziToast.destroy();
		toastSuccess(__("编辑成功"), __("您的评论已编辑"));
		$("body,html").animate({
