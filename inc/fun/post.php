...
//文末附加内容
function argon_get_additional_content_after_post(){
	global $post;
	$postID = $post -> ID;
...
//页面浏览量
function argon_get_post_views($post_id){
	$count_key = 'views';
...
add_action('get_header', 'argon_set_post_views');
//字数和预计阅读时间
function argon_get_article_words($str){
...
function argon_get_article_words_total($str){
	$res = argon_get_article_words($str);
	return $res['cn'] + $res['en'] + $res['code'];
}
function argon_get_reading_time($len){
...
//获取文章 Meta
function argon_get_article_meta($type){
	if ($type == 'sticky'){
	...
	if ($type == 'views'){
		if ( get_option( "argon_view_counter" ) == 'wp-statistics' && function_exists( 'wp_statistics_pages' ) ) {
			...
			$views = argon_get_post_views( get_the_ID() );
		}
...
//获取文章字数统计和预计阅读时间
function argon_get_article_reading_time_meta($post_content_full){
	$post_content_full = apply_filters("argon_html_before_wordcount", $post_content_full);
	$words = argon_get_article_words($post_content_full);
	...
	$res .= ' ' . argon_get_article_words_total($post_content_full) . " " . __("字", 'argon');
	$res .= '</span>
		</div>
		<div class="post-meta-devide">|</div>
		<div class="post-meta-detail post-meta-detail-words">
			<i class="fa fa-hourglass-end" aria-hidden="true"></i>
			' . argon_get_reading_time(argon_get_article_words($post_content_full)) . '
		</div>
	';
	return $res;
}
//当前文章是否隐藏 阅读时间 Meta
function argon_is_readingtime_meta_hidden(){
...
//根据文章 id 获取标题
function argon_get_post_title_by_id($id){
	return get_post($id) -> post_title;
}
//获取顶部 Banner 背景图（用户指定或必应日图）
function argon_get_banner_background_url(){
...
function argon_the_content_filter($content){
	if (get_option('argon_enable_lazyload') != 'false'){
...
add_filter('the_content' , 'argon_the_content_filter',20);
//文章过时信息显示
