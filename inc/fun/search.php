...
function argon_search_filter($query) {
...
add_filter('pre_get_posts', 'argon_search_filter');

/**
 * 修复 WordPress 搜索结果为空，返回为 200 的问题。
 * @author ivampiresp <im@ivampiresp.com>
 */
function argon_search_404_fix_template_redirect()
{
    if (is_search()) {
        global $wp_query;
...
add_action('template_redirect', 'argon_search_404_fix_template_redirect');
