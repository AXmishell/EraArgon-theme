...
function argon_update_post_meta_ajax(){
	if (!isset($_POST['argon_meta_box_nonce'])){
		return;
	}
	$nonce = $_POST['argon_meta_box_nonce'];
...
add_action('wp_ajax_update_post_meta_ajax' , 'argon_update_post_meta_ajax');
//安全加固：移除 nopriv 钩子，禁止未登录用户通过 AJAX 修改文章 meta
function argon_regenerate_ai_post_summary(): void {
	if (!isset($_POST['argon_meta_box_nonce'])){
		return;
	}
	$nonce = $_POST['argon_meta_box_nonce'];
