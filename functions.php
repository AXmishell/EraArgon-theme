...
// 导航菜单 Walker 类
require get_template_directory() . '/inc/fun/nav-walkers.php';
...
function argon_theme_locale_hook( $locate, $domain ) {
	if ( $domain == 'argon' ) {
...
add_filter( 'theme_locale', 'argon_theme_locale_hook', 10, 2 );
...
function argon_array_remove( &$arr, $item ) {
...
argon_session_init();
//add_action('init', 'argon_session_init');
...
//发送邮件
function argon_send_mail( $to, $subject, $content ) {
	wp_mail( $to, $subject, $content, array( 'Content-Type: text/html; charset=UTF-8' ) );
}

function argon_check_email_address( $email ) {
