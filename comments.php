			...
			<?php
				if (get_option("argon_comment_pagination_type") == "page"){
					if (get_comment_pages_count() > 1){
						echo argon_formatted_comment_paginate_links_for_all_platforms();
					}
				}else{
					$prevPageUrl = argon_comment_paginate_links_prev_url();
		...
		<form>
			...
			<div class="row hide-on-comment-editing" style="margin-bottom: -10px;">
				...
				<div class="<?php echo $col3_class;?>">
					<div class="form-group">
						<div class="input-group input-group-alternative mb-4 post-comment-captcha-container" captcha="<?php echo argon_get_comment_captcha(argon_get_comment_captcha_seed());?>">
							<div class="input-group-prepend">
								<span class="input-group-text"><i class="fa fa-key"></i></span>
							</div>
							<input id="post_comment_captcha" class="form-control" placeholder="<?php _e('验证码', 'argon');?>" type="text" <?php if (current_user_can('level_7')) {echo('value="' . argon_get_comment_captcha_answer(argon_get_comment_captcha_seed()) . '" disabled');}?>>
							<style>
